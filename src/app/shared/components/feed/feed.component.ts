...
export class FeedComponent implements AfterViewInit {
  ...
  async decrypt(id) {
    ...
    try {
      ...
      const amIallowed = await this.dappInjectorService.config.contracts['superfluid'
  ...
  ngAfterViewInit(): void {
    this.store.pipe(web3Selectors.isviewReady).subscribe(async (value) => {
      ...
      for (let k = this.totalSupply; k >= 1; k--) {
        ...
        const profile:ProfileStructStruct = await this.readingContract.contract.getProfile(k);
        let imageError = false;
        let image_Profile;
        let json_Profile;
        console.log(k)
        const profil_image_cid = profile.imageURI.replace('https://ipfs.io/ipfs/','');
        const profil_json_cid  = profile.followNFTURI.replace('https://ipfs.io/ipfs/','');
          try {
        ...
        for (let i = 1; i <= nrPubs; i++) {
          try {
            ...
            if(pubjson.encrypted == true){
              ...
              this.publications.push({ ...{ id: encrpyted_index, description: 'This content is protected only for subscribers',src: 'assets/images/encryption.jpg', encrypted:true },...{profile:{handle:'encrpyted'}},...{profile_src:image_Profile} });
               encrpyted_index++;
            } else {
